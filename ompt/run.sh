...
CASE_DIR=${CASE_DIR:-$OUTPUT_ROOT/$CASE_NAME}
CSV_DIR=${CSV_DIR:-$CASE_DIR/csv}
SVG_DIR=${SVG_DIR:-$CASE_DIR/svg}
TXT_DIR=${TXT_DIR:-$CASE_DIR/txt}

mkdir -p "$CSV_DIR" "$SVG_DIR" "$TXT_DIR"

OUTPUT=${OUTPUT:-$SVG_DIR/amr.svg}
TIMING_CSV=${TIMING_CSV:-$CSV_DIR/ompt_regions.csv}
TIMING_SVG=${TIMING_SVG:-$SVG_DIR/ompt_timing.svg}
